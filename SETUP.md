...
- **DocentesCO**: Edicion, Dia, Horario, Desde, Hasta, Docente, Staff, Observaciones, Usuario, Id — cada fila es UN período (Desde-Hasta) de una edición; una misma edición puede tener varios períodos con distinto docente/staff a lo largo del tiempo. Edición: Pau, Sofía y Paula (`paula.arigos@institutoilce.com`), y SuperAdmin. El resto solo puede ver.
- **CredencialesZoom** (opcional): Sala, Usuario, Contrasena, IdReunion — las 8 salas ya están cargadas fijas en el código (pestaña "Credenciales Zoom", visible para todos), aunque el ID de reunión solo lo tengo cargado para Sala 1 y Sala 5 (los otros 6 me faltan). Si en algún momento cambia una contraseña o querés cargar el ID de las que faltan, se puede cargar acá y esa versión pisa a la fija. No hace falta crear esta pestaña para que funcione.
