...
export default function CronogramaCMPage() {
  ...
  async function agregar() {
    ...
    try {
      ...
      const dia = idxSemana < 5 ? DIAS_SEMANA[idxSemana] : (idxSemana === 5 ? 'SABADO' : 'DOMINGO');
      const res = await fetchAutenticado('/api/cronograma-cm', {
                      ...
                      return (
                        <td key={f} className="border border-border align-top p-1 min-w-[130px]">
                          {items.map((a) => {
                            ...
                            return (
                              <div key={a.id} className={`rounded-md px-2 py-1 text-[11px] font-semibold mb-1 border-l-2 ${color.bg} ${color.text} ${color.border}`}>
                                {a.tipo}
