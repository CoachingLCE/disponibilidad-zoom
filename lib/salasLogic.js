...
export function calcularEdicionesFinalizadas(historico) {
  ...
  Object.keys(grupos).forEach((key) => {
    ...
    const fin = new Date(inicio); fin.setDate(inicio.getDate() + (total - 1) * 7);
    if (fin.toISOString().slice(0, 10) < hoyISO) finalizadas.add(key);
  });
...
export function calcularFormaciones(clases) {
  ...
  return Object.keys(grupos).map((key) => {
    ...
    // Cuatrimestre: en Ontológico (48 clases) se cursa en 3 bloques de 16, con 2 semanas de
    // receso entre cada uno — Clase 1-16 = 1er cuatrimestre, 17-32 = 2do, 33-48 = 3ro.
    // Para cursos de 16 clases en total, es un solo cuatrimestre completo.
    const cuatrimestre = total ? Math.min(Math.ceil(cargadas / 16), Math.ceil(total / 16)) || 1 : null;

    return { codigo, edicion: numero, numero, fechaInicio, fechaFinal, estado, cargadas, total, pct, proximaTxt, cuatrimestre };
