...
export function calcularFormaciones(clases) {
  const grupos = {};
  // Antes solo se contaban las clases con fecha puntual (las que pasaron por "Reservar").
  // Las cargadas por el horario recurrente semanal (Grilla de Salas Zoom, sin fecha exacta,
  // solo día+hora) quedaban afuera del todo — por eso esas formaciones no aparecían acá.
  clases.forEach((c) => {
  ...
  return Object.keys(grupos).map((key) => {
    ...
    const cargadas = new Set(grupo.map((c) => c.numero).filter(Boolean)).size;
    ...
    return { codigo, edicion, fechaInicio, fechaFinal, estado, cargadas, total, pct, proximaTxt };
