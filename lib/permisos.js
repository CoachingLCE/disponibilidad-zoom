...
// Editar Docentes y Staff de C.O: permiso aparte de los roles generales, específico por
// persona — Pau, Sofía y Diego (SuperAdmin). El resto del equipo puede ver la pestaña
// pero no asignar ni modificar docentes/staff.
const EMAILS_EDITAN_DOCENTES_CO = ['sofia.salgueiro@institutoilce.com', 'paula.arigos@institutoilce.com'];

export function tienePermisoEditarDocentesCO(usuario) {
  if (!usuario) return false;
  if (tieneAlguno(usuario, ['SuperAdmin'])) return true;
  return EMAILS_EDITAN_DOCENTES_CO.includes((usuario.email || '').toLowerCase());
}
...
// Ver el Historial de acciones (Auditoría) y el detalle completo de Análisis: reservado a
// Admin/SuperAdmin — Colaborador y Educativo ven el resto de la app normalmente, pero estas
// dos pantallas/secciones les muestran el mensaje de "sin acceso" en vez del contenido.
export function tienePermisoAuditoria(usuario) {
  return tieneAlguno(usuario, ['Admin', 'SuperAdmin']);
}

// "Ver como": previsualizar la app tal cual la ve otra persona (mismo criterio que Accesos).
