...
export function SessionProvider({ children }) {
  const [usuarioReal, setUsuario] = useState(null);
  const [token, setToken] = useState(null);
  const [cargando, setCargando] = useState(true);
  // "Ver como": un Admin/SuperAdmin puede previsualizar la app tal cual la ve otra persona
  // (por ejemplo Colaborador o Educativo), sin cerrar su propia sesión — para revisar qué
  // ve/puede hacer cada rol. `usuario` (lo que usa el resto de la app) pasa a ser esa persona
  // mientras dure la previsualización; `usuarioReal` sigue siendo quien realmente inició sesión.
  ...
  async function fetchAutenticado(url, options = {}) {
    // En modo "Ver como" no se manda NINGÚN cambio de verdad (evita guardar o borrar algo
    // real mientras se está previsualizando cómo ve la app otra persona) — se corta acá,
    // antes de llegar al servidor, y se devuelve un error prolijo que cada pantalla ya sabe
    // mostrar (mismo formato { error } que devuelve cualquier API de la app).
    const metodo = (options.method || 'GET').toUpperCase();
    if (verComo && metodo !== 'GET') {
      return new Response(
        JSON.stringify({ error: `Estás viendo la app como ${verComo.nombre} — salí del modo vista (✕ arriba a la derecha) para poder guardar cambios.` }),
  ...
  return (
    <SessionContext.Provider value={{ usuario, usuarioReal, verComo, setVerComo, token, login, logout, cargando, fetchAutenticado }}>
      {children}
