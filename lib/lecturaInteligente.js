...
// Palabras "de corte": si aparecen después del nombre de un docente/staff, quiere decir
// que ya empezó el próximo dato (otra etiqueta del formulario) — el nombre no debe
// seguir comiéndoselas. Sin este freno, "Docente Diego Lerner Staff Ana Gomez" capturaba
// "Diego Lerner Staff Ana" como si todo fuera el nombre del docente.
const PALABRAS_CORTE_NOMBRE = [
  'staff', 'docente', 'profe', 'profesor', 'profesora', 'sala', 'horario', 'hora', 'horas',
  'hs', 'edicion', 'ed', 'cuatrimestre', 'cantidad', 'alumnos', 'alumnas', 'personas',
  'inscriptos', 'inscriptas', 'observaciones', 'tematica', 'temporada', 'fecha', 'curso', 'materia'
].join('|');
...
function normalizar(s) {
  return (s || '')
    ...
    // OJO: el punto (.) se mantiene igual que los dos puntos (:) \u2014 es el separador de
    // hora en formatos tipo "18.00 hs". Si se lo sacaba ac\u00e1 (como pasaba antes), un texto
    // como "Horario: 18.00 horas" quedaba "18 00 horas", el detector de hora agarraba
    // "00 horas" como si fuera la hora (0:00 en vez de 18:00) y el "18" que quedaba
    // suelto se colaba despu\u00e9s como si fuera el n\u00famero de Edici\u00f3n \u2014 sin serlo.
    .replace(/[^a-z0-9\s:.,]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
export function interpretarTexto(texto, hoy = new Date()) {
  ...
  // --- Hora: "18:00", "18.00", "18,00", "18 hs", "18hs", "18 horas" ---
  ...
  // --- Docente: "profe X", "profesor X", "docente X", o con dos puntos: "Docente: X"
  // (muy común al pegar un texto ya formateado con etiquetas, ej. "Docente:\nJuan Perez") ---
  // Hasta 4 palabras de nombre, pero corta antes de la próxima etiqueta del formulario
  // (Staff, Sala, Horario, etc.) para no comérsela como si fuera parte del nombre.
  // (el freno de corte también aplica a la PRIMERA palabra: si después de "Docente:" viene
  // directo otra etiqueta vacía, ej. "Docente:\nStaff: Ana", no hay que agarrar "Staff"
  // como si fuera el nombre del docente)
  m = restante.match(new RegExp(`\\b(?:profe|profesor|profesora|docente)\\s*:?\\s*(?!(?:${PALABRAS_CORTE_NOMBRE})\\b)([a-z]+(?:\\s+(?!(?:${PALABRAS_CORTE_NOMBRE})\\b)[a-z]+){0,3})`));
  if (m) {
  ...
  // --- Staff: "staff X" o "Staff: X" ---
  m = restante.match(new RegExp(`\\bstaff\\s*:?\\s*(?!(?:${PALABRAS_CORTE_NOMBRE})\\b)([a-z]+(?:\\s+(?!(?:${PALABRAS_CORTE_NOMBRE})\\b)[a-z]+){0,3})`));
  if (m) {
    const nombreCapturado = nombreValido(m[1]);
