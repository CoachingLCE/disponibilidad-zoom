...
export async function leerClases() {
  ...
  return filas
    ...
    .map((f) => ({
      ...
      // Se puede reservar una Formación sin elegir sala todavía (por ejemplo, si las 8 están
      // ocupadas en ese horario y no se quiere perder el lugar en el cronograma) — queda
      // marcada "pendiente" hasta que alguien le asigne una sala de verdad. Si la pestaña
      // "Clases" del Sheet todavía no tiene esta columna agregada, queda en false y no rompe
      // nada (mismo criterio que Sala/Cuatrimestre en Docentes C.O.).
      pendienteSala: f.PendienteSala === 'TRUE',
...
/**
 * Guarda (o corrige) a mano la Fecha de inicio de una edición puntual en la pestaña
 * "Formaciones" del Sheet — pensado para corregir un error desde el detalle de una
 * clase en Cronograma, sin depender de que alguien edite el archivo de código
 * lib/fechasInicioReales.js. Si ya hay una fila para ese Codigo+Edicion se actualiza
 * (conservando FechaFinal/Estado/MesesCertificacion si ya los tenía cargados); si no
 * existe, se agrega una fila nueva.
 */
export async function escribirFechaInicioFormacion(codigo, edicion, fechaInicio) {
  const filas = await readSheet('Formaciones');
  const existente = filas.find((f) => f.Codigo === codigo && String(f.Edicion || '') === String(edicion || ''));
  const fila = {
    Codigo: codigo,
    Edicion: String(edicion || ''),
    FechaInicio: fechaInicio || '',
    FechaFinal: existente?.FechaFinal || '',
    Estado: existente?.Estado || '',
    MesesCertificacion: existente?.MesesCertificacion || ''
  };
  if (existente) await patchRow('Formaciones', existente._rowIndex, fila);
  else await appendRow('Formaciones', fila);
}
...
function filaClase(clase) {
  return {
    ...
    PendienteSala: clase.pendienteSala ? 'TRUE' : '',
    Id: clase.id
  };
}
...
export async function eliminarClasePorId(id) {
  ...
  await patchRow('Clases', objetivo._rowIndex, {
    ...
    Label: '', Duracion: '', Fecha: '', Docente: '', Staff: '', Tematica: '', Observaciones: '', PendienteSala: '', Id: ''
  });
  return true;
}
...
export async function leerPostergaciones() {
  ...
  return filas.filter((f) => f.Codigo).map((f) => ({
    ...
    dia: f.Dia || '', horaMin: f.HoraMin ? parseInt(f.HoraMin, 10) : null,
    sala: f.Sala || '', duracion: f.Duracion ? parseInt(f.Duracion, 10) : null,
